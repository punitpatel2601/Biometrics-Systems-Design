...
import numpy as np
import scipy
def gabor_filter(im, orient, freq, block_size2=16, kx=0.65, ky=0.65):
    """
    ...
    # Helder: added block_size2 parameter to see if only the central portion
    # of the image is used for minutiae extraction. 
    # Basides some slightly differences it only show bigger squares on the singularities image
    ...
    # Helder: where is block_size2 was 16 in both loops
    for i in range(0,rows//block_size2):
        for j in range(0,cols//block_size2):
    ...
    for k in range(0, np.shape(finalind)[1]):
        ...
        # Helder: where is block_size2 was 16
        return_img[r][c] = np.sum(img_block * gabor_filter[int(orientindex[r//block_size2][c//block_size2]) - 1])
